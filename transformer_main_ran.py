...
def get_age_transformer(device, num_classes, age_interval, min_age, max_age, mid_feature_size):
	...
	transformer = TransformerModel(
		...
		mode='context').to(device) #'mean'

	if PRETRAIN_FULL:
		pretrained_model = AgeTransformer(backbone, transformer, num_features, mid_feature_size)

		# Use this for starting from backbone weights trained for e2e task with transformer:
		
		pretrained_model_file = os.path.join(pretrained_model_path, "weights.pt")
		transformer.load_state_dict(torch.load(pretrained_model_file), strict=False)

		#print(pretrained_model)

		#num_features = pretrained_model.backbone.num_features

		# backbone = pretrained_model.backbone
		# backbone.train()
		# backbone.to(device)

		linear_mid = pretrained_model.linear
		linear_mid.train()
		linear_mid.to(device)

		transformer = pretrained_model.transformer
		transformer.train()
		transformer.to(device)

		age_transformer = AgeTransformer(backbone, transformer, num_features, mid_feature_size)
	
		age_transformer.linear = linear_mid
	else:
		age_transformer = AgeTransformer(backbone, transformer, num_features, mid_feature_size)
...
if __name__ == "__main__":
	...
	### Train ###
	writer = SummaryWriter('logs/Morph2/transformer/encoder/' + cur_time_str + 'bin_1_layers_4_heads_4_lr' + str(lr) + '_batch_8_copies_10_mid_feature_size_1024_augs_at_val_context4') #_context_true_iter_warmup_10000_amp_batchnorm_after_encoder_iter_15e5_no_encoder_only_mean') #_RS')
	# writer = None

	model_path = 'weights/Morph2/transformer/encoder/' + cur_time_str + 'bin_1_layers_4_heads_4_lr' + str(lr) + '_batch_8_copies_10_mid_feature_size_1024_augs_at_val_context4' #_context_true_iter_warmup_10000_amp_batchnorm_after_encoder_iter_15e5_no_encoder_only_mean_RS'
